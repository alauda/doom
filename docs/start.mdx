...
执行 `yarn doom dev` 启动开发服务，浏览器会自动打开文档首页

### 生产环境构建

执行 `yarn doom build` 构建生产环境代码，构建完成后会在 `dist` 目录生成静态文件

### 本地预览

执行 `yarn doom serve` 预览构建后的静态文件

### 使用脚手架模板

执行 `yarn doom new` 使用脚手架模板生成项目、模块或文档

### 翻译文档

执行 `yarn doom translate` 将 `<root>/zh` 中文文档自动翻译到 `<root>/en`
...
:::warning
请在执行导出操作前先执行 `yarn doom build` 构建操作
:::

执行 `yarn doom export` 导出文档为 PDF 文件，支持 `-n, --name` 参数指定导出文件名前缀，最终导出的文档文件名为 `<name>-<lang>.pdf`。

导出功能依赖 [`playwright`](https://playwright.dev)，可以设置如下环境变量加速下载：
