...
Options:
  ...
  -g, --glob <path...>     Glob pattern for source files
  -h, --help               display help for command
```

`-g, --glob` 参数可以指定需要翻译的文件路径，支持 `glob` 语法（`.md,.mdx` 后缀可省略），注意参数值必须带引号否则会被命令行解析造成非预期行为

例如 `yarn translate -g 'abc/**/*' 'xyz/**/*'`，将在 `<root>/<source>/abc`, `<root>/<source>/xyz`
目录下的所有文件翻译到 `<root>/<target>/abc`, `<root>/<target>/xyz` 目录下，未指定时默认翻译整个 `<root>/<source>` 目录
