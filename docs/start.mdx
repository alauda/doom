...
- `-C, --copy` 参数可选，是否复制本地路径的资源文件到目标目录，默认为 `false`，即改变资源文件的引用路径为引用源路径。示例：
  1. `/zh/abc.jpg` 翻译时将复制 `<root>/public/zh/abc.jpg` 到 `<root>/public/en/abc.jpg`，并修改文档中的引用路径为 `/en/abc.jpg`
  ...
  4. 如果没有启用 `-C, --copy` 参数, `<root>/<source>/abc.mdx` 文档中的 `./assets/<source>/xyz.jpg` 翻译时，将修改文档中的引用路径为 `../<source>/assets/<target>/xyz.jpg`
