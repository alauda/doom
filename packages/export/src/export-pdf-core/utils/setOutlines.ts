import { PDFRef, type PDFDocument } from 'pdf-lib'
...
export interface PDFOutlineItemWithChildren extends PDFOutlineItem {
  children?: PDFOutlineInfo[]
  open?: boolean
}

export type PDFOutlineInfo = PDFOutlineItem | PDFOutlineItemWithChildren

const walk = (
  outlines: readonly PDFOutlineInfo[],
  // eslint-disable-next-line @typescript-eslint/no-invalid-void-type
  callback: (outline: PDFOutlineInfo) => void | boolean, // stop walking to children if returned false
) => {
  for (const outline of outlines) {
    const ret = callback(outline)
    if ('children' in outline && outline.children?.length && ret !== false) {
      walk(outline.children, callback)
    }
  }
}

const flatten = (outlines: readonly PDFOutlineInfo[]) => {
...
const getOpeningCount = (outlines: readonly PDFOutlineInfo[]) => {
  let count = 0
...
export const setOutlines = (
  doc: PDFDocument,
  outlines: readonly PDFOutlineInfo[],
) => {
  // Refs
  const rootRef = doc.context.nextRef()
  const refMap = new WeakMap<PDFOutlineInfo, PDFRef>()
  ...
  const createOutline = (
    ...
    for (let i = 0; i < length; i += 1) {
      ...
      const childrenDict = (() => {
        if ('children' in outline && outline.children?.length) {
